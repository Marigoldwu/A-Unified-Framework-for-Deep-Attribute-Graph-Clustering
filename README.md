...
> 💡 **Tips:**
>
> - The arguments marked in <span style="color: red">red</span> are usually need to be specified.
> - The arguments marked in <span style="color: green">green</span> are the drawing functions.
> - The arguments marked with <span style="color: blue">blue</span> are related to the data loading.
> - The argument marked with <span style="color: purple">purple</span> is strongly recommended to you to record the experimental key points.
...
🚌 `Step 1`: Make sure that your dataset are well processed and the file suffix is 'npy' which denotes the file store the numpy array. If your dataset is graph data, you need to include {dataset name}\_feat.npy、{dataset name}\_label.npy、{dataset name}\_adj.npy. If your dataset is non-graph data, there are two ways to handle. One is directly using {dataset name}\_feat.npy、{dataset name}\_label.npy, and set the type of constructing graph in `data_processor.py`. Another is to construct graph data manually, and use {dataset name}\_feat.npy、{dataset name}\_label.npy、{dataset name}\_adj.npy, but you need remember what value the k used because the dataset is considered as graph dataset.
