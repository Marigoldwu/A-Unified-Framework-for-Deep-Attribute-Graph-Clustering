...
class HardSampleAwareNetwork(nn.Module):
    def __init__(self, input_dim, hidden_dim, act, n_num):
        super(HardSampleAwareNetwork, self).__init__()
        ...
        self.alpha.data = torch.tensor(0.99999).cuda()

        self.pos_weight = torch.ones(n_num * 2).cuda()
        self.pos_neg_weight = torch.ones([n_num * 2, n_num * 2]).cuda()
